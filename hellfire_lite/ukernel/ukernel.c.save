...
/*
EDF RT scheduler (Earliest Deadline First)
*/
uint8_t HF_TaskEDFReschedule(void){
	uint8_t schedule;
	uint8_t i=0;
	uint16_t j=0xffff;
	...
	for(i=1;i<=HF_max_index;i++){
		...
		if ((HF_task_entry->ptask) && (HF_task_entry->period > 0)){
			if ((HF_task_entry->status == TASK_READY) || (HF_task_entry->status == TASK_NOT_RUN)){
				if ((HF_task_entry->next_tick_count - HF_task_entry->period + HF_task_entry->deadline < j) && (HF_task_entry->capacity_counter > 0)){
					j = HF_task_entry->next_tick_count - HF_task_entry->period + HF_task_entry->deadline;
					schedule = i;
				}
				if (--HF_task_entry->priority == 0){
					HF_task_entry->next_tick_count += HF_task_entry->period;
